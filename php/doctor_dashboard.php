...
$consultations_sql = "
    SELECT k.*, a.Nama AS Nama_Anak 
    FROM konsultasi k 
    JOIN anak a ON k.ID_Anak = a.ID_Anak 
    WHERE k.ID_Dokter=(SELECT ID_Dokter FROM dokter WHERE ID_User='$user_id') 
    ORDER BY k.Tanggal_Respon DESC 
    LIMIT 5";
$consultations_result = $conn->query($consultations_sql);

// Fetch latest payment reports
$payments_sql = "
    SELECT p.*, o.Nama_Ayah, o.Nama_Ibu 
    FROM pembayaran p 
    JOIN orang_tua o ON p.ID_Orang_Tua = o.ID_Orang_Tua 
    WHERE p.ID_Dokter=(SELECT ID_Dokter FROM dokter WHERE ID_User='$user_id') 
...
<body>
    ...
    <div class="content">
        <h1>Selamat datang, Dr. <?php echo $doctor['Nama']; ?></h1>
        <div class="profile-icon">
            <a href="doctor_profile.php">
                <?php if (!empty($doctor['Foto'])) { ?>
                    <img src="<?php echo $doctor['Foto']; ?>" alt="Profile">
                <?php } else { ?>
        ...
        <div class="consultations">
            ...
            <ul>
                <?php while ($consultation = $consultations_result->fetch_assoc()) { ?>
                    <li>
                        <strong><?php echo $consultation['Tanggal_Respon']; ?>:</strong>
                        <?php echo $consultation['Pesan']; ?> -
                        <em><?php echo $consultation['Nama_Anak']; ?></em>
                    </li>
                <?php } ?>
            </ul>
        </div>
        <div class="payments">
            <h3>Laporan Pembayaran Terbaru</h3>
            <ul>
                <?php while ($payment = $payments_result->fetch_assoc()) { ?>
                    <li>
                        <strong><?php echo $payment['Tanggal_Pembayaran']; ?>:</strong>
                        Rp<?php echo number_format($payment['Jumlah_Pembayaran'], 2, ',', '.'); ?> -
                        <em><?php echo $payment['Nama_Ayah']; ?> / <?php echo $payment['Nama_Ibu']; ?></em>
                    </li>
                <?php } ?>
            </ul>
        </div>
