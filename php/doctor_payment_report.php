...
$sql = "SELECT p.*, o.Nama_Ayah, o.Nama_Ibu FROM pembayaran p
        JOIN orang_tua o ON p.ID_Orang_Tua = o.ID_Orang_Tua
        WHERE p.ID_Dokter=(SELECT ID_Dokter FROM dokter WHERE ID_User='$doctor_id')";
...
function getTotalEarnings($conn, $doctor_id, $interval)
{
    $sql = "SELECT SUM(Jumlah_Pembayaran) as total FROM pembayaran 
            WHERE ID_Dokter=(SELECT ID_Dokter FROM dokter WHERE ID_User='$doctor_id') 
            AND Tanggal_Pembayaran >= DATE_SUB(NOW(), INTERVAL $interval)";
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    return $row['total'] ? $row['total'] : 0;
}

$daily_earnings = getTotalEarnings($conn, $doctor_id, '1 DAY');
$monthly_earnings = getTotalEarnings($conn, $doctor_id, '1 MONTH');
?>
...
<script>
    ...
    function filterPayments() {
        ...
        window.location.href = `doctor_payment_report.php?start_date=${startDate}&end_date=${endDate}`;
    }
</script>
...
<body>
    <div class="sidebar">
        ...
        <div class="menu-container">
            ...
            <a href="doctor_consultation.php">
                ...
                Konsultasi</a>
            <a href="doctor_payment_report.php" class="active">
    ...
    <div class="content">
        ...
        <div class="top-container">
            ...
            <div class="earnings">
                <p>Total Pemasukan Harian: <span>Rp<?php echo number_format($daily_earnings, 2, ',', '.'); ?></span></p>
                <p>Total Pemasukan Bulanan: <span>Rp<?php echo number_format($monthly_earnings, 2, ',', '.'); ?></span></p>
            </div>
        </div>
        ...
        <table>
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr><td>" . $row['Nama_Ayah'] . "</td><td>" . $row['Nama_Ibu'] . "</td><td>Rp" . number_format($row['Jumlah_Pembayaran'], 2, ',', '.') . "</td><td>" . $row['Tanggal_Pembayaran'] . "</td></tr>";
            }
            ?>
        </table>
    </div>
