...
function getChildGrowthStatus($child_id, $conn)
{
    $sql = "SELECT * FROM hasil_gizi WHERE ID_Anak='$child_id' ORDER BY Tanggal_Pemeriksaan DESC LIMIT 1";
    $result = $conn->query($sql);
    return $result->fetch_assoc();
}

function getGrowthHistory($child_id, $conn)
{
    $sql = "SELECT * FROM riwayat_pertumbuhan WHERE ID_Anak='$child_id' ORDER BY Tanggal_Pemeriksaan DESC LIMIT 5";
    $result = $conn->query($sql);
    return $result->fetch_all(MYSQLI_ASSOC);
}

function getRecentConsultations($parent_id, $conn)
{
    $sql = "SELECT k.Tanggal_Respon, d.Nama AS Dokter, a.Nama AS Anak, k.Status 
            FROM konsultasi k 
            JOIN dokter d ON k.ID_Dokter = d.ID_Dokter 
            JOIN anak a ON k.ID_Anak = a.ID_Anak 
            WHERE a.ID_Orang_Tua = '$parent_id' 
            ORDER BY k.Tanggal_Respon DESC LIMIT 5";
    return $conn->query($sql)->fetch_all(MYSQLI_ASSOC);
}

$parent_id = $conn->query("SELECT ID_Orang_Tua FROM orang_tua WHERE ID_User='$user_id'")->fetch_assoc()['ID_Orang_Tua'];
$recent_consultations = getRecentConsultations($parent_id, $conn);
?>
...
<body>
    ...
    <div class="content">
        <h1>Selamat datang, Mom <?php echo $user['Nama']; ?></h1>
        <div class="profile-icon">
            <a href="parent_profile.php">
                <?php if (!empty($user['Foto'])) { ?>
                    <img src="<?php echo $user['Foto']; ?>" alt="Profile" class="profile-img">
                <?php } else { ?>
        ...
        <div class="dashboard-container">
            ...
            <?php foreach ($children as $child) : ?>
                <div class="dashboard-card">
                    <h3>Statistik Anak: <?php echo $child['Nama']; ?></h3>
                    <?php
                    $growth_status = getChildGrowthStatus($child['ID_Anak'], $conn);
                    if ($growth_status) {
                        echo "<p>Status Gizi: " . $growth_status['Status_Gizi'] . "</p>";
                        echo "<p>Tanggal Pemeriksaan Terakhir: " . $growth_status['Tanggal_Pemeriksaan'] . "</p>";
                    } else {
                        echo "<p>Belum ada data status gizi.</p>";
                    }

                    $growth_history = getGrowthHistory($child['ID_Anak'], $conn);
                    if ($growth_history) {
                        $dates = [];
                        $weights = [];
                        $heights = [];
                        foreach ($growth_history as $history) {
                            $dates[] = $history['Tanggal_Pemeriksaan'];
                            $weights[] = $history['Berat_Badan'];
                            $heights[] = $history['Tinggi_Badan'];
                        }
                    ?>

                        <div class="chart-container">
                            <canvas id="growthChart-<?php echo $child['ID_Anak']; ?>"></canvas>
                        </div>
                        <script>
                            const ctx<?php echo $child['ID_Anak']; ?> = document.getElementById('growthChart-<?php echo $child['ID_Anak']; ?>').getContext('2d');
                            new Chart(ctx<?php echo $child['ID_Anak']; ?>, {
                                type: 'bar',
                                data: {
                    ...
                    <?php
                        ...
                        echo "<p>Belum ada riwayat pertumbuhan.</p>";
                    }
                    ?>
                </div>
            <?php endforeach; ?>
            ...
            <div class="dashboard-card">
                ...
                <?php if (count($recent_consultations) > 0) : ?>
                    <ul>
                        <?php foreach ($recent_consultations as $consultation) : ?>
                            <li><?php echo $consultation['Tanggal_Respon']; ?>: Konsultasi dengan Dr. <?php echo $consultation['Dokter']; ?> untuk anak <?php echo $consultation['Anak']; ?> - Status: <?php echo $consultation['Status']; ?></li>
                        <?php endforeach; ?>
                    </ul>
                <?php else : ?>
