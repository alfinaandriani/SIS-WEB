...
$sql_pending = "SELECT k.*, a.Nama AS Nama_Anak, a.Foto FROM konsultasi k JOIN anak a ON k.ID_Anak = a.ID_Anak WHERE k.ID_Dokter=(SELECT ID_Dokter FROM dokter WHERE ID_User='$doctor_id') AND k.Status='Pending'";
...
$sql_completed = "SELECT k.*, a.Nama AS Nama_Anak, a.Foto FROM konsultasi k JOIN anak a ON k.ID_Anak = a.ID_Anak WHERE k.ID_Dokter=(SELECT ID_Dokter FROM dokter WHERE ID_User='$doctor_id') AND k.Status='Selesai'";
            ...
            <div class="consultation-cont">
                ...
                <div class="consultation-cards">
                    ...
                    while ($row = $result_pending->fetch_assoc()) {
                        ...
                        echo '<h3>' . $row['Nama_Anak'] . '</h3>';
                        echo '<p>' . $row['Pesan'] . '</p>';
                        echo '<button onclick="window.location.href=\'doctor_consultation_view.php?consultation_id=' . $row['ID_Konsultasi'] . '\'">Detail</button>';
                        echo '</div>';
                    }
                    ?>
                </div>
